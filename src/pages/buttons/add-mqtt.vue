<template>
  <f7-page>
    ...
    <f7-list no-hairlines-md>
      ...
      <f7-list-input
        :value="mqttTopic"
        @input="mqttTopic = $event.target.value"
        label="Topic"
        type="text"
        placeholder="Enter a MQTT topic"
        required
        validate
        error-message="The MQTT topic is required"
        clear-button
      ></f7-list-input>
      <f7-list-input
        :value="message"
...
export default {
  ...
  mounted() {
    if (this.$f7route.params.btn_id) {
      ...
      .then((response) => {
        ...
        this.mqttTopic = button.mqtt_topic;
  ...
  methods: {
    save: function() {
      if (this.name && this.orderHor && this.orderVer && this.selectedColor && this.mqttTopic && this.message) {
        let data = {
    ...
    catchIr: function() {
      if (this.selectedCatchNode) {
        ...
        this.$socket.emit('catch_ir', JSON.stringify({'node_id': this.selectedCatchNode, 'int_limit': false}));
      }
    },
    test: function() {
